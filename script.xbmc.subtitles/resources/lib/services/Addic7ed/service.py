...
def query_TvShow(name, season, episode, file_original_path, langs):
    ...
    for subs in soup("td", {"class":"NewsTitle", "colspan" : "3"}):
      try:
        ...
        try:
            hiImage = subsFullTable.find("img", {"title": "Hearing Impaired"})
            hearing_imp = (hiImage != None)
        except:
            hearing_imp = False
        ...
        status = statusTD.getText().strip()
        downloadButtonLink = statusTD.findNext("td").find("a")
        link = "%s%s"%(self_host,downloadButtonLink["href"])
        if status == "Completed" and (lang in langs) :
            sublinks.append({'filename':"%s.S%.2dE%.2d-%s" %(name.replace("_", ".").title(), int(season), int(episode),subteams ),'link':link,'language_name':fullLanguage,'language_id':lang,'language_flag':"flags/%s.gif" % (lang,),'movie':"movie","ID":"subtitle_id","rating":"0","format":"srt","sync":hashed,"hearing_imp":hearing_imp})
      except:
        print "Error"
        pass
    return sublinks
